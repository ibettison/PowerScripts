...
#>
  # Your script here
    ...
    if(!(Test-Path -Path "$env:SystemDrive\Temp\Protected\")) {
      New-Item -Path "$env:SystemDrive\Temp\Protected\" -ItemType 'directory' 
    }
      ...
      ($credentials).Password | ConvertFrom-SecureString -Key $AESKey | Out-File -FilePath $FilePath
  ...
  Check-IfPathExists $secureCredentials
