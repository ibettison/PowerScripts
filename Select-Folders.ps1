 function Select-Groups
 {
   ...
   try 
   {
     ...
     Start-Job -Name getList -ScriptBlock {param([Parameter(Mandatory,HelpMessage='Enter the Partial Group')][String]$pGroup) Get-ADGroup -Filter {Name -like $pGroup} -Properties * | Select-Object -Property Name, Description, SamAccountName} -ArgumentList $ADGroup | Out-Null
     ...
     else
     {
       ...
       $group = $(foreach ($item in $groupList){
           ...
           e={$Idx}}, Name, Description, SamAccountName
       $Idx++	}) |
