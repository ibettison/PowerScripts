    ...
    Foreach ($TargetFolder in $TargetFolders)
        ...
        {        
            if ($MissingFolder.SideIndicator -eq "=>")
            {
                ...
                Write-Progress -Activity "Removing Folder" -Status "Removing Folders"
                try
                {
        ...
        ForEach ($TargetFile in $FilteredTargetFiles)
        {
            ...
            If ($MatchingSourceFile -ne $Null)
            {
                If ($MatchingSourceFile.LastWriteTime -gt $TargetFile.LastWriteTime)
                {
                    ...
                    Write-Progress -Activity "Updating File" -Status "Updating File"
                    try
                    {
...
Write-Progress -Activity "Sync Process ended" -Status "Ready"
